...
class Parser {
	...
	/**
	 * @param \Solr\Domain\Result\Explanation\Explain $explain
	 */
	public function injectExplain(\Solr\Domain\Result\Explanation\Explain $explain) {
	...
	protected function getNodeFromName($tokenName) {
		if(mb_strpos($tokenName,'sum of:') !== false || mb_strpos($tokenName,'result of:') !== false)  {
			return new \Solr\Domain\Result\Explanation\Nodes\Sum();
		}

		if(mb_strpos($tokenName,'product of:') !== false) {
			return new \Solr\Domain\Result\Explanation\Nodes\Product();
		}

		if(mb_strpos($tokenName,'max') !== false && mb_strpos($tokenName,'of:')) {
			return new \Solr\Domain\Result\Explanation\Nodes\Max();
		}

			//when nothing else matched we have a leaf node
		return new \Solr\Domain\Result\Explanation\Nodes\Leaf();
	}
	 ...
	 */
	public function parse(	\Solr\Domain\Result\Explanation\Content $content,
							\Solr\DOmain\Result\Explanation\MetaData $metaData) {
